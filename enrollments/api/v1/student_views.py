...
class StudentCourseDetailView(generics.RetrieveAPIView):
    ...
    def get_serializer_context(self):
        ...
        if not course_id or not user.is_authenticated:
            return context
        context.update(lesson_progress_context(user, course_id))
        return context
...
class StudentLessonDetailView(generics.RetrieveAPIView):
    ...
    def get_queryset(self):
        return student_lessons_queryset(self.request.user)

    def get_serializer_context(self):
        context = super().get_serializer_context()
        lesson = self.get_object()
        context.update(lesson_progress_context(self.request.user, lesson.course.pk))
        return context
