...
| Student Lesson | `GET /api/v1/student/lessons/{id}/`; `POST /api/v1/student/lessons/{id}/start/` and `/complete/` | Active enrollment in a Published course; start/complete also require an available lesson | Empty body for actions | Safe lesson detail and progress; `lesson_locked`, `student_lesson_not_found` |
